...
/*   Updated: 2024/01/05 20:18:59 by ybelatar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_match(char c, va_list *parameters, int *length, int fd)
{
	...
	if (c == 'u')
		return (ft_putnbrbasebis(va_arg(*parameters, unsigned int), 10,
				length, fd));
	if (c == 'c')
	...
	if (c == 'p')
		return (ft_putptrhexabis((uintptr_t)va_arg(*parameters, void *),
				length, fd));
	if (c == 'x')
		return (ft_putnbrbasebis(va_arg(*parameters, unsigned int), 16,
				length, fd));
	if (c == 'X')
		return (ft_putnbrbasebis(va_arg(*parameters, unsigned int), 17,
				length, fd));
	if (c == '%')
